...
def run_smoke(token: str) -> None:
    ...
    config_response = requests.get(f"{BASE_URL}/v1/config", headers=headers, timeout=30)
    config_response.raise_for_status()
    config = config_response.json()
    if set(config.get("endpoints", [])) != EXPECTED_ENDPOINTS:
        raise AssertionError(f"Unexpected Iceberg REST endpoints: {config.get('endpoints')}")
