...
Maintenance tests use prepared tables with multiple snapshots, data files,
manifests, and orphan candidates in Parquet and ORC.

```text
Maintenance and Planning Tests =
  compatible([Multi-File and Multi-Snapshot Prepared Tables] x [Test Cases Below])
```

| Test case | Expected behavior |
|---|---|
| Expire snapshots | Only eligible snapshots and files are removed; current rows and metadata remain valid. |
| Rewrite data files | Selected files are compacted while preserving every live row. |
| Rewrite manifests | Manifest layout changes without changing table rows or snapshot semantics. |
| Remove orphan files | Unreferenced files are removed while every live file remains available. |
| Metadata tables | File, manifest, snapshot, and sequence rows describe the current physical state. |
| Filter planning | Scan tasks include the files required by the filter and exclude unrelated files. |
| Compaction planning | Files are grouped according to the table layout and requested maintenance operation. |
