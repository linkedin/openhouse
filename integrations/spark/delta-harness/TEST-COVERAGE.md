...
Merge-on-read adds prepared tables whose row-level changes are represented by
position-delete files. The shared DML contracts run on unpartitioned,
date-partitioned, and replacement-lineage tables in that write mode.

Focused behavior verifies creation and persistence of delete files, switching
write modes, reads and writes while delete files are live, metadata and changelog
visibility, replication, time travel, rollback, and maintenance operations that
retain or compact the physical delete state.

## Branches and write-audit-publish

Reference coverage follows branches and tags through creation, scoped writes,
merge, fast-forward, cherry-pick, divergence, and deletion. The same references
are combined with time travel, rename, maintenance, table evolution, storage
formats, and merge-on-read delete files.

Write-audit-publish coverage separates staged work from the table's current
state. It verifies configuration and session routing, staged inserts and DML,
independent staging identifiers, publication through cherry-pick and
`publish_changes`, and rejection of repeated publication or an expired staged
snapshot.
