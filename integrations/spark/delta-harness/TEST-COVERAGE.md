...
Compatibility coverage asks whether independent clients and evolving tables
continue to observe one coherent state. Concurrent writers and catalog locks must
either commit valid changes or report a conflict without losing another writer's
work.

Streaming writes must resume from checkpoints without duplicating or dropping
rows. Table evolution must preserve compatible readers and writers across schema
and property changes. Writer-version checks keep the same contract visible when
the client runtime changes.
