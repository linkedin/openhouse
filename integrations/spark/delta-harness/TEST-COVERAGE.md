...
Governance coverage verifies that access-control grants and revocations change who
can use a table without changing its data. Column tags and encryption properties
must remain visible through catalog operations, and file-replication settings must
retain their intended destination.

The embedded environment does not provide the policy service needed to enforce
grant and revoke decisions. Those assertions run in the acceptance environment
instead of being weakened for local execution.
