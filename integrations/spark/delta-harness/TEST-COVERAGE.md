...
Column-default tests use prepared tables with rows written before a defaulted
column is added.

```text
Column Default Tests =
  compatible([Pre-Existing Rows in Parquet and ORC] x [Test Cases Below])
```

| Test case | Expected behavior |
|---|---|
| SQL parser | Adding a column with a supported default expression succeeds. |
| Persisted schema | Iceberg schema metadata records the column default. |
| Existing rows | Rows written before the column existed read with the default value. |
| Low-level read | The Iceberg read path returns the same default value as the SQL path. |
