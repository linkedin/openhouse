...
Governance tests use prepared tables with access policies, column tags,
encryption settings, and replication destinations in Parquet and ORC.

```text
Governance Tests =
  compatible([Governed Prepared Tables] x [Test Cases Below])
```

| Test case | Expected behavior |
|---|---|
| Grant access | The granted principal can perform the authorized table operation. |
| Revoke access | The revoked principal is denied while the table and its data remain unchanged. |
| Column tags | Tags remain attached to the intended columns through catalog operations. |
| Default encryption | New data uses the configured encryption behavior and remains readable through OpenHouse. |
| Replication destination | Setting, changing, and removing replication records the requested destination without changing table rows. |

Access-control assertions run in li-openhouse because embedded OpenHouse does
not provide the policy service.
