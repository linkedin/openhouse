...
Catalog DDL coverage follows tables and namespaces through creation, rejection,
rename, and later writes. Partition transforms, sort orders, and write-distribution
settings must appear in catalog-visible metadata and remain effective when data is
written.

Invalid definitions must fail before leaving an unusable namespace, side table, or
