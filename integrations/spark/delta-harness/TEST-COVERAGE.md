...
Schema tests use nested-value and schema-evolution prepared tables in Parquet and
ORC.

```text
Schema Tests =
  compatible([Nested and Evolving Prepared Tables] x [Test Cases Below])
```

| Test case | Expected behavior |
|---|---|
| Struct values | Struct fields, nested structs, and nullability survive writes and reads. |
| Array values | Array order, elements, and nullability survive writes and reads. |
| Map values | Map keys, values, and nullability survive writes and reads. |
| Add and remove columns | Catalog metadata and query results expose the new schema while preserving unaffected values. |
| Rename and reorder fields | Values remain attached to their logical columns after metadata changes. |
| Widen a type | Compatible widening preserves existing values and accepts values in the wider range. |
| Reject an incompatible change | The operation fails and preserves the prior schema and rows. |
| Delete by a nested field | The matching rows are removed without corrupting the nested values. |
