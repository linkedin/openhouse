...
Column-default coverage separates parser acceptance from persisted behavior. It
checks whether adding a defaulted column records the default in table metadata and
whether rows written before the column existed read with the intended value.

The same behavior is examined through SQL, Iceberg schema serialization, and the
low-level read path. This exposes a gap where the SQL syntax is accepted even when
the default is not persisted or applied.
