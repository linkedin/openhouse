...
trait SurfaceScenarios extends ScenarioKit {
  ...
  /** An unseeded, empty unpartitioned table in the given file format. */
  ...
  def surfaceReaderCases(format: String): List[Plan.Case] =
    List(
      surfaceStreamReadCase(format),
      surfaceStreamWriteCase(format),
  ...
  def surfaceRewriteProcedureCases(format: String): List[Plan.Case] =
    List(
      surfaceProcRewriteManifestsCase(format))

  /** ancestors_of lists both snapshots of the table's two-snapshot history. */
  private def surfaceProcAncestorsOfCase(format: String): Plan.Case =
    surfaceTwoSnapshotPreparation(format).test("surface.proc.ancestorsOf") { table =>
  ...
  def surfaceSnapshotProcedureCases(format: String): List[Plan.Case] =
    List(
      surfaceProcAncestorsOfCase(format),
  ...
  def surfaceMetadataCases(format: String): List[Plan.Case] =
    List(
      surfaceMetaHiddenColumnsCase(format),
  ...
  /** The schema changes Iceberg allows and the ones the catalog rejects. */
  def surfaceSchemaCases(format: String): List[Plan.Case] =
    List(
      surfaceSchemaRelaxNotNullCase(format),
      surfaceSchemaDecimalWidenCase(format),
      surfaceSchemaNestedAddFieldCase(format),
      surfaceSchemaNestedDropFieldCase(format),
  ...
  def surfaceWriteCases(format: String): List[Plan.Case] =
    List(
      surfaceWriteDistributionHashCase(format),
  ...
  /** CREATE VIEW and ANALYZE TABLE COMPUTE STATISTICS are each rejected with an exception. */
  private def surfacePinViewsAnalyzeCase(format: String): Plan.Case =
    surfaceBasePreparation(format).test("surface.pin.viewsAnalyze") { table =>
      Check.intercept[Exception](
        table.spark.sql(
  ...
  def surfacePinCases(format: String): List[Plan.Case] =
    List(
      surfacePinImportProcsCase(format),
      surfacePinViewsAnalyzeCase(format))
}
