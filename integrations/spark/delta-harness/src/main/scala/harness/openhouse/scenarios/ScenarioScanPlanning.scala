 ...
 */
trait ScenarioScanPlanning extends MaintenanceTableFixtures {

  /** The split-size case, one file format at a time. */
  lazy val scanPlanningCases: List[TestCase] =
    fileFormats.map(format =>
      TestCase(s"scanPlanning.splitSize @ $format", splitSizeCase(format)))
  ...
  private def splitSizeCase(format: String)(ctx: Ctx): Unit = {
    ...
    withOwnedTable(spark.sql(_), table)(
      ...
      try {
        ...
        // byte length, so a split-size below one file keeps one task group per file while a split-size above the
        // whole table combines everything into one group.
        ...
        // The split size is session-scoped Spark configuration, so the case restores the value the session held.
