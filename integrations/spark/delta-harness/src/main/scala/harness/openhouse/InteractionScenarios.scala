...
trait InteractionScenarios extends ScenarioKit {
  ...
  def interactionDdlCases(format: String): List[Plan.Case] = {
    ...
    List(
      preparation.test(
        "interact.ddl.ttAfterAddColumn",
        "After ADD COLUMN and an insert into the new column, time travel to the pre-DDL snapshot " +
          "reads the old schema with 3 rows, while a current read sees the new column.") { table =>
      ...
      preparation.test(
        "interact.ddl.restoreAfterAddColumn",
        "Rolling back to the pre-DDL snapshot after ADD COLUMN and an insert keeps the evolved " +
          "schema, restores 3 rows reading null for the new column, and the table still accepts " +
          "writes into that column.") { table =>
        val seedSnapshotId = snapshotIds(table.spark, table.name).last
        table.spark.sql(
          s"ALTER TABLE ${table.name} ADD COLUMN extra_col INT")
        table.spark.sql(
      ...
      preparation.test(
        "interact.ddl.dropColAfterData",
        "DROP COLUMN on a column that holds data is rejected, the column's data remains readable, " +
          "and the table remains writable.") { table =>
        table.spark.sql(
          s"ALTER TABLE ${table.name} ADD COLUMN extra_col INT")
        table.spark.sql(
          s"INSERT INTO ${table.name} VALUES $extraColInsert9")
  ...
  def interactionMiscellaneousCases(
    ...
    List(
      basePreparation.test(
        "interact.maint.compactEvolved",
        "Compacting a table after an ADD COLUMN and inserts into the new column preserves all " +
          "rows, the new column's non-null values, and null for rows written before the column " +
          "was added.") { table =>
        table.spark.sql(
          s"ALTER TABLE ${table.name} ADD COLUMN extra_col INT")
        table.spark.sql(
