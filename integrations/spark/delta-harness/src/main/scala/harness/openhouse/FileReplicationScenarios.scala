 ...
 * on the files a commit produces.
 *
...
trait FileReplicationScenarios extends ScenarioKit {
  ...
  lazy val fileReplicationCases: List[Plan.Case] =
    List(
      Plan.Case("fileReplication.outputFileProperty @ core", outputFilePropertyCase))
  ...
  private def outputFilePropertyCase(ctx: Ctx): Unit = {
    ...
    withOwnedTable(spark.sql(_), table)(
      ...
      assert(keys == Seq(1L, 2L, 3L), s"rows wrong after write: $keys")
    }
  }
