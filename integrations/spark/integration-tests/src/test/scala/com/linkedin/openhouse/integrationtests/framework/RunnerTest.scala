...
final class RunnerTest {
  ...
  def configuredParallelismRequiresAPositiveInteger(): Unit = {
    ...
    assertEquals(
      4,
      RunnerConfiguration.parallelism(Map("INTEGRATION_TEST_PARALLELISM" -> "4"), 8))
      ...
      val failure = assertThrows(
        ...
        () =>
          RunnerConfiguration.parallelism(
            Map("INTEGRATION_TEST_PARALLELISM" -> value),
            availableProcessors = 8))
      assertEquals(
        s"INTEGRATION_TEST_PARALLELISM must be a positive integer; received '$value'",
        failure.getMessage)
    }
  }
