...
/**
 * The `com.linkedin.openhouse.integrationtests.Main` launch class used by the embedded OpenHouse Gradle tasks.
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      println(s"\n=== OpenHouse OpenHouse integration tests ($header) ===\n")
      ...
      if (failed > 0 || passed == 0) {
        throw new AssertionError(
          s"OpenHouse integration tests finished with $passed passed, $skipped skipped, and $failed failed cases")
      }
    } catch {
