 ...
 * (commit) count before and after, so a test can assert the change in both data and commits (e.g. "a no-match UPDATE
...
object PreparedTable {
  private[integrationtests] def currentRows[S <: Schema](spark: SparkSession, table: String, schema: S): Seq[Row] = {
  ...
  private[integrationtests] def snapshotCount(spark: SparkSession, table: String): Long =
...
private[integrationtests] object OwnedTableLifecycle {
  /**
...
object TableTest {
  ...
  private[integrationtests] def nextQualifiedTableName(namespace: String): String =
