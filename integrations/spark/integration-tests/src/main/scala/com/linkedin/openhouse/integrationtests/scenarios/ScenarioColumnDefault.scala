...
// The integration-test `Schema` is a table-shape trait, so the Iceberg schema type is aliased here.
...
trait ScenarioColumnDefault extends ColumnDefaultTableFixtures {
  ...
  private def readApplyProbe(ctx: Ctx): Unit = {
    ...
    if (nestedFieldBuilder.isEmpty) {
      ...
      // share nothing. The token comes from the test table-name generator, which is UUID plus a counter.
