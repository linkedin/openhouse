...
Catalog DDL tests use namespace and table prepared states in Parquet and ORC.

```text
Catalog DDL Tests =
  compatible([Namespace and Table Prepared States] x [Test Cases Below])
```

| Test case | Expected behavior |
|---|---|
| Create namespace | The namespace becomes visible with its requested metadata. |
| Reject invalid namespace or table definitions | The operation reports the invalid definition and leaves no partial object behind. |
| Create table | The table is readable with the requested schema and properties. |
| Rename table | The new name resolves to the same table identity and existing data. |
| Partition transforms | Catalog metadata records the transform and later writes use the resulting partition layout. |
| Sort order | Catalog metadata records the order and new files follow it. |
| Write distribution | The requested distribution mode affects new file layout without changing logical rows. |
