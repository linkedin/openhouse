...
These tests use prepared tables with concurrent writers, catalog locks,
checkpoints, schema changes, property changes, and different writer versions.

```text
Compatibility and Streaming Tests =
  compatible([Concurrent, Locked, Evolving, Streaming Prepared Tables]
    x [Test Cases Below])
```

| Test case | Expected behavior |
|---|---|
| Concurrent writers | Each successful commit remains visible; a conflicting writer reports a conflict instead of losing committed work. |
| Catalog lock | Only the lock owner performs the protected change, and release restores access for later operations. |
| Streaming checkpoint recovery | A restarted stream resumes from its checkpoint without duplicating or dropping rows. |
| Schema evolution compatibility | Compatible readers and writers continue across supported schema changes. |
| Property evolution compatibility | Supported property changes preserve readable table state and later writes. |
| Writer-version compatibility | Supported client versions produce the same committed table behavior. |
