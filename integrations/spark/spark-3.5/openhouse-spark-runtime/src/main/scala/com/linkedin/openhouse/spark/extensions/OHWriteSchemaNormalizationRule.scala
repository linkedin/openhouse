...
class OHWriteSchemaNormalizationRule(spark: SparkSession) extends Rule[LogicalPlan] {
  ...
  private def projectByName(
    ...
    // cast to the target type if necessary (replicating ResolveOutputRelation's Cast insertion).
      ...
      targetCols.map { tgt =>
        srcByLower.get(tgt.name.toLowerCase) match {
          case Some(src) if src.name == tgt.name && src.dataType == tgt.dataType =>
            src // name and type already match — keep as-is
          case Some(src) =>
            val castExpr =
              if (src.dataType == tgt.dataType) src
              else Cast(src, tgt.dataType, tz)
