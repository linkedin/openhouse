...
class OHCaseInsensitiveResolveRule(spark: SparkSession) extends Rule[LogicalPlan] {
   ...
   *       the same plan are excluded from the top-level map. [[resolveOperatorsDown]] +
   *       [[transformExpressions]] walks the whole plan tree and cannot distinguish which
   *       [[UnresolvedAttribute]] belongs to which catalog. Renaming a name that also exists in a
   *       Hive/other-catalog relation would corrupt resolution for those references under
