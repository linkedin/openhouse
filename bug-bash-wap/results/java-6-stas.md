...
void testEmptySnapshotFastForwardChain() throws Exception {
  try (SparkSession spark = getSparkSession()) {
    spark.sql("CREATE TABLE openhouse.d1.test_java6 (id int, data string)");
    Operations operations = Operations.withCatalog(spark, null);
    Table table = operations.getTable("d1.test_java6");
    
    // Your test implementation here
    
    spark.sql("DROP TABLE openhouse.d1.test_java6");
  }
}
```
