...
-- Step 1: Setup
CREATE TABLE openhouse.d1.test_sql7_${timestamp} (name string);
ALTER TABLE openhouse.d1.test_sql7_${timestamp} SET TBLPROPERTIES ('write.wap.enabled'='true');
...
```sql
-- Snapshots query
SELECT snapshot_id, operation, summary FROM openhouse.d1.test_sql7_xxx.snapshots;
-- Result: [paste output]

-- Refs query
SELECT name, snapshot_id FROM openhouse.d1.test_sql7_xxx.refs;
-- Result: [paste output]

-- Data verification
SELECT * FROM openhouse.d1.test_sql7_xxx;
-- Result: [paste output]
```
