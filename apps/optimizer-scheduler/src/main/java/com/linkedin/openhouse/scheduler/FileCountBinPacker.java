 ...
 * from the per-table {@link TableStats} passed alongside the operation list; operations whose stats
 * are missing or whose snapshot reports no file count are treated as zero.
 *
...
public class FileCountBinPacker implements BinPacker {
  ...
  @Override
  public List<Bin> pack(List<TableOperation> pending, Map<String, TableStats> statsByUuid) {
    if (pending.isEmpty()) {
      return List.of();
    }

    // Project once: each operation's packing cost is just a long.
    Map<String, Long> costByOperationId =
        pending.stream()
            .collect(Collectors.toMap(TableOperation::getId, op -> cost(op, statsByUuid)));
  ...
  private static long cost(TableOperation op, Map<String, TableStats> statsByUuid) {
    TableStats stats = statsByUuid.get(op.getTableUuid());
